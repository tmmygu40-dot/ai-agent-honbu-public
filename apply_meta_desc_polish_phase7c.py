#!/usr/bin/env python3
# coding: utf-8
"""Phase 7-C description polish (80-99字 候補 1,529件、C1/C2 分割).

Phase 7-B 案A継承。差分:
- ターゲット帯: 80 <= len(desc) < 100（1,529件）
- C1/C2 分割: hash(slug+"phase7c_split")%2 で安定分割（C1=734 / C2=795）
...
使い方:
  python apply_meta_desc_polish_phase7c.py --lane c1 --dry-run
  python apply_meta_desc_polish_phase7c.py --lane c2 --dry-run
"""
...
PAD_THRESHOLD = 100  # base 平均 86字 → PAD は base<71 のときのみ発火
HARD_MAX = 145
...
def lane_of(slug):
    return "c1" if stable_idx(slug, "phase7c_split", 2) == 0 else "c2"
...
def main():
    ...
    ap.add_argument("--lane", choices=["c1", "c2", "all"], default="all")
    ...
    print(f"=== Phase 7-C lane={args.lane} {'DRY-RUN' if args.dry_run else 'EXECUTE'} 完了 ===")
    ...
    rep_json = os.path.join(REPORT_DIR, f"meta_desc_polish_phase7c_{args.lane}_result.json")
